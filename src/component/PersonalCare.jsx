...
function PersonalCare() {
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["homeAppliances"],
    queryFn: async () => {
      const res = await fetch(`http://localhost:4000/personal care`);
  ...
  return (
    <Box pt="6">
      <HStack pl="6" py="5" backgroundColor="gray.100">
        <Text fontSize={{lg:'17px', md:'17px', sm:'12px'}}  fontWeight="500">
          Best Selling Personal care items | 3hr Instant Delivery
        </Text>{" "}
        <Button
          size="xs"
          px="2"
